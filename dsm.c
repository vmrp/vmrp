...
{
	if (showApiLog)
		LOG("mr_plat(code:%d, param:%d)", code, param);
...
{
	if (showApiLog)
		LOG("mr_platEx(code:%d, il:%d)", code, input_len);
...
static char filenamebuf[DSM_MAX_FILE_LEN + 5] = {0}; //因为要连接其它几个路径，所以长度要更长
...
{
	//if(showApiLog) printf("mr_drawBitmap(bmp:0x%08x, x:%d, y:%d, w:%d, h:%d)", bmp, x, y, w, h);
}
				 ...
				 uint8 cr, uint8 cg, uint8 cb)
{
	...
	if (showApiLog)
		LOG("mr_DrawRect(x:%d, y:%d, w:%d, h:%d)",
			x, y, w, h);
}
...
{
	if (showApiLog)
		LOG("mr_TestCom(l%d, p0:%d, p1:%d)", L, input0, input1);
	return MR_SUCCESS;
}
...
int32 mr_unzip(uint8 *inputbuf, int32 inputlen, uint8 **outputbuf, int32 *outputlen)
{
	...
	if (!inputbuf || inputlen <= 0 || !outputbuf || inputbuf[0] != 0x1f || inputbuf[1] != 0x8b)
		return MR_FAILED;
